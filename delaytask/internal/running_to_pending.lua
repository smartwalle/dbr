...
local consumerKey = KEYS[4]
local now = tonumber(ARGV[1])

local retryMessage = function(messageKey)
    -- 判断消息结构是否存在
    ...
    -- 获取消费者
    local consumer = redis.call('HGET', messageKey, 'consumer') or ''
    if consumer ~= '' then
        -- 获取消费者的有效时间
    ...
    if retryRemain > 0 then
        ...
        -- 获取消息uuid
        local uuid = redis.call('HGET', messageKey, 'uuid') or ''
        if  uuid ~= '' then
            -- 添加到[待消费队列]中
